...
<p align="center">
  <code>NDT 2.11.0</code> · <code>12 项可分发资产</code> · <code>16 个配方</code> · <code>MCP 2.5.0</code> · <code>Apache-2.0</code>
</p>
...
本次发布把公开发行包和 NDT 核心架构基线统一为 `2.11.0`。
...
- 新增 `ai-app-ui` 与架构图/重绘合同，覆盖 Agent 状态、信任边界、响应式行为、语义图型选择以及 Mermaid/draw.io 的保真重绘。
- 视觉评分与生产检查共用适用性和当前文件证据规则；总分不能绕过缺失的产物、预览、内容或可访问性证据。

公开目录目前包含 12 项可实际分发的可复用资产、9 个公开安全案例、59 条辅助资料和 16 个配方。规则、工具、快照和兼容 ID 单独分类，不再全部计入“设计资产”。
...
| 路由 | 典型任务 |
|---|---|
| `frontend-ui` | Dashboard、工作台、响应式 UI、交互审阅 |
...
- 随包公开前端版本为 `0.4.3`，已对照私有前端 `0.5.11` 真源重新核对。原生桌面封装、Registry 写入界面、私有预览媒体及其本地视觉外壳不进入公开包。
- 公开发行版有意保持为非权威派生版。
